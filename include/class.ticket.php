...
class Ticket {
    ...
    var $pickup_datetime;
    var $pickup_address;
    var $airport_terminal;
    var $airport_airline;
    var $airport_flight_number;
    var $airport_dest_orig;
    var $destination;
    var $driver_id;
    ...
    function getUpdateInfo() {
        ...
        $info=array('name'  =>  $this->getName(),
                    ...
                    'is_booking'  =>  $this->getIsBooking(),
                    'booking_type'  =>  $this->getBookingType(),
                    'pickup_datetime'  =>  $this->getPickupDatetime(),
                    'pickup_address'  =>  $this->getPickupAddress(),
                    'airport_terminal'  =>  $this->getAirportTerminal(),
                    'airport_airline'  =>  $this->getAirportAirline(),
                    'airport_flight_number'  =>  $this->getAirportFlightNumber(),
                    'airport_dest_orig'  =>  $this->getAirportDestOrig(),
    ...
    //GetTaxi setters/getters
    function getIsBooking() {
        return $this->ht['is_booking'];
    }

    function getPickupDatetime() {
        return $this->ht['pickup_datetime'];
    }

    function getPickupAddress() {
        return $this->ht['pickup_address'];
    }

    function getAirportTerminal() {
        return $this->ht['airport_terminal'];
    }

    function getAirportAirline() {
        return $this->ht['airport_airline'];
    }

    function getAirportFlightNumber() {
        return $this->ht['airport_flight_number'];
    }

    function getAirportDestOrig() {
        return $this->ht['airport_dest_orig'];
    }

    function getDestination() {
        return $this->ht['destination'];
    }

    function getBookingType() {
        return $this->ht['booking_type'];
    }

    function getDriverID() {
        return $this->ht['driver_id'];
    }

    function setIsBooking($is_booking) {
        return $this->is_booking = $is_booking;
    }

    function setPickupDatetime($pickup_datetime) {
        return $this->pickup_datetime = $pickup_datetime;
    }

    function setPickupAddress($pickup_address) {
        return $this->pickup_address = $pickup_address;
    }

    function setAirportTerminal($airport_terminal) {
        return $this->airport_terminal = $airport_terminal;
    }

    function setAirportAirline($airport_airline) {
        return $this->airport_airline = $airport_airline;
    }

    function setAirportFlightNumber($airport_flight_number) {
        return $this->airport_flight_number = $airport_flight_number;
    }

    function setAirportDestOrig($airport_dest_orig) {
        return $this->airport_dest_orig = $airport_dest_orig;
    }
    ...
    function setBookingType($booking_type) {
        return $this->booking_type = $booking_type;
    }

    function setDriverID($driver_id) {
   ...
   function onOverdue($whine=true, $comments="") {
    ...
    function update($vars, &$errors) {
        ...
        //GetTaxi fields
        $fields['is_booking']             = array('type'=>'int',        'required'=>1,                            'error'=>'Type of ticket required');
        $fields['booking_type']           = array('type'=>'string',     'required'=>$vars['is_booking'] == 1?1:0, 'error'=>'Booking type required');
        if ($vars['is_booking'] == 1 && $vars['booking_type'] == 'airport') $mandatory = true; else $mandatory = false;
        $fields['pickup_datetime']        = array('type'=>'string',     'required'=>$vars['is_booking'] == 1?1:0, 'error'=>'Pickup date is required');
        $fields['pickup_address']         = array('type'=>'string',     'required'=>$vars['is_booking'] == 1?1:0, 'error'=>'Pickup address is required');
        $fields['airport_terminal']       = array('type'=>'string',     'required'=>$mandatory == true?1:0, 'error'=>'Airport terminal is required');
        $fields['airport_airline']        = array('type'=>'string',     'required'=>$mandatory == true?1:0, 'error'=>'Airline is required');
        $fields['airport_flight_number']  = array('type'=>'string',     'required'=>$mandatory == true?1:0, 'error'=>'Flight number is required');
        $fields['airport_dest_orig']      = array('type'=>'string',     'required'=>$mandatory == true?1:0, 'error'=>'Flight origin or destination is required');
        $fields['destination']            = array('type'=>'string',     'required'=>$vars['is_booking'] == 1?1:0, 'error'=>'Destination is required');
        $fields['driver_id']              = array('type'=>'string',     'required'=>0, 'error'=>'Driver ID is required');
        ...
        $sql='UPDATE '.TICKET_TABLE.' SET updated=NOW() '
            ...
            .' ,is_booking='.db_input($vars['is_booking'])
            .' ,booking_type='.db_input($vars['booking_type'])
            .' ,pickup_datetime='.db_input($vars['pickup_datetime'])
            .' ,pickup_address='.db_input($vars['pickup_address'])
            .' ,airport_terminal='.db_input($vars['airport_terminal'])
            .' ,airport_airline='.db_input($vars['airport_airline'])
            .' ,airport_flight_number='.db_input($vars['airport_flight_number'])
            .' ,airport_dest_orig='.db_input($vars['airport_dest_orig'])
            .' ,destination='.db_input($vars['destination'])
            .' ,driver_id='.db_input($vars['driver_id']);
   ...
   /*============== Static functions. Use Ticket::function(params); ==================*/
    ...
    function create($vars, &$errors, $origin, $autorespond=true, $alertstaff=true) {
        ...
        //GetTaxi field
        if (strtolower($origin) == 'web' || strtolower($origin) == 'staff') {
            $fields['is_booking']             = array('type'=>'int',        'required'=>1, 'error'=>'Type of ticket required');
            $fields['booking_type']           = array('type'=>'string',     'required'=>$vars['is_booking'] == 1?1:0, 'error'=>'Booking type required');
            if ($vars['is_booking'] == 1 && $vars['booking_type'] == 'airport') $mandatory = true; else $mandatory = false;
            $fields['pickup_datetime']        = array('type'=>'string',     'required'=>$vars['is_booking'] == 1?1:0, 'error'=>'Pickup date is required');
            ...
            $fields['airport_dest_orig']      = array('type'=>'string',     'required'=>$mandatory == true?1:0, 'error'=>'Flight origin or destination is required');
            $fields['destination']            = array('type'=>'string',     'required'=>$vars['is_booking'] == 1?1:0, 'error'=>'Destination is required');
            $fields['message']                = array('type'=>'text',       'required'=>$vars['is_booking'] == 1?0:1, 'error'=>'Message required');
            $fields['driver_id']              = array('type'=>'string',     'required'=>0, 'error'=>'Driver ID is required');
        }
